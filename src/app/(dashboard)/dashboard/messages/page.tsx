...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    setTplLoading(true);
    api
      .get<{ items?: OrgTemplate[] } | OrgTemplate[]>("/templates")
      .then((res) => {
        setTplLoading(false);
        if (res.error) return;
        const raw = res.data;
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    if (ext === "csv") {
      ...
      Papa.parse<Record<string, string>>(file, {
        ...
        skipEmptyLines: true,
        complete: (results) => {
          const contacts: CsvContact[] = results.data
            .map((row) => {
              const keys = Object.keys(row).map((k) => k.toLowerCase());
              const get = (terms: string[]) => {
                const k = keys.find((k) => terms.some((t) => k.includes(t)));
                return k
                  ? row[Object.keys(row)[keys.indexOf(k)]]?.trim() || undefined
                  : undefined;
              };
              return {
                name: get(["name", "full"]),
                email: get(["email", "e-mail", "mail"]),
                phone: get(["phone", "mobile", "tel", "cell"]),
              };
            })
            .filter((c) => c.email || c.phone);
          if (!contacts.length)
            setCsvError(
              "No valid contacts found. Ensure the file has email or phone columns.",
            );
      ...
      const contacts: CsvContact[] = rows
        .map((row) => {
          ...
          const get = (terms: string[]) => {
            const k = keys.find((k) => terms.some((t) => k.includes(t)));
            return k
              ? String(row[Object.keys(row)[keys.indexOf(k)]])?.trim() ||
                  undefined
              : undefined;
          };
          return {
      ...
      if (!contacts.length)
        setCsvError(
          "No valid contacts found. Ensure the sheet has email or phone columns.",
        );
      setCsvContacts(contacts);
    } else {
      setCsvError(
  ...
  const removeMember = (id: string) =>
    setSelectedMembers((s) => s.filter((m) => m.id !== id));
  ...
  const recipientSummary = () => {
    ...
    return (
      RECIPIENT_GROUPS.find((r) => r.id === form.recipientGroup)?.label ??
      "All Members"
    );
  };
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="mb-3 flex overflow-hidden rounded-xl border border-[#E3D4C2]">
                      {(["group", "specific", "csv"] as RecipientMode[]).map(
                        (mode) => {
                            ...
                            <button
                              ...
                              className={`flex flex-1 items-center justify-center gap-1.5 border-r py-2 text-[11px] font-medium last:border-r-0 border-[#E3D4C2] transition-colors ${
                                active
                                  ...
                                  : "bg-white text-[rgba(26,26,26,0.55)] hover:bg-[#FAF7F3]"
                              }`}
                            >
                              {icons[mode]}
                              {labels[mode]}
                    ...
                    {recipientMode === "specific" && (
                      <div className="space-y-2">
                        {selectedMembers.length > 0 && (
                          <div className="flex flex-wrap gap-1.5">
                            {selectedMembers.map((m) => (
                              <span
                                key={m.id}
                                className="flex items-center gap-1 rounded-full border border-[#C8A061]/30 bg-[rgba(200,160,97,0.12)] px-2.5 py-1 text-xs text-[#1F1C18]"
                              >
                                {memberLabel(m)}
                                <button
                                  type="button"
                                  onClick={() => removeMember(m.id)}
                                  className="ml-0.5 text-[rgba(26,26,26,0.4)] hover:text-[#8E0E00]"
                                >
                        ...
                        <div className="relative">
                          ...
                          <input
                            ref={memberInputRef}
                            className="cs-input w-full pl-8"
                          ...
                          {searchingMem && (
                            <Spinner
                              size="sm"
                              className="absolute right-3 top-1/2 -translate-y-1/2 text-[#C8A061]"
                            />
                          )}
                            ...
                            <div className="absolute z-20 mt-1 max-h-48 w-full overflow-y-auto rounded-xl border border-[#E3D4C2] bg-white shadow-lg">
                              {memberResults.map((m) => (
                                ...
                                >
                                  ...
                                  <div>
                                    <p className="font-medium leading-none text-[#1F1C18]">
                                      {memberLabel(m)}
                                    </p>
                                    {m.email && (
                                      <p className="mt-0.5 text-[11px] text-[rgba(26,26,26,0.45)]">
                                        {m.email}
                                      </p>
                                    )}
                                  </div>
                          ...
                          {showMemDrop &&
                            ...
                            !searchingMem && (
                              <div className="absolute z-20 mt-1 w-full rounded-xl border border-[#E3D4C2] bg-white px-4 py-3 text-sm text-[rgba(26,26,26,0.5)] shadow-lg">
                                No members found matching &ldquo;{memberQuery}
                                &rdquo;
                              </div>
                            )}
                        </div>
                    ...
                    {recipientMode === "csv" && (
                      <div className="space-y-3">
                        ...
                        />
                        {csvError && (
                          <p className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-[#8E0E00]">
                            {csvError}
                          </p>
                        )}
                          ...
                          <div className="overflow-hidden rounded-xl border border-[#E3D4C2]">
                            ...
                            <div className="max-h-36 divide-y divide-[#E3D4C2] overflow-y-auto">
                              {csvContacts.slice(0, 50).map((c, i) => (
                                <div
                                  key={i}
                                  className="flex gap-4 px-3 py-1.5 text-xs hover:bg-[rgba(212,175,106,0.04)]"
                                >
                                  <span className="w-28 truncate text-[rgba(26,26,26,0.7)]">
                                    {c.name ?? "—"}
                                  </span>
                                  <span className="flex-1 truncate text-[#1F1C18]">
                                    {c.email ?? "—"}
                                  </span>
                                  <span className="w-24 truncate text-[rgba(26,26,26,0.7)]">
                                    {c.phone ?? "—"}
                                  </span>
                                </div>
                              ))}
                              {csvContacts.length > 50 && (
                                <p className="px-3 py-1.5 text-xs text-[rgba(26,26,26,0.4)]">
                                  …and {csvContacts.length - 50} more
                                </p>
                              )}
                            </div>
                          </div>
                        ...
                        <p className="text-[11px] text-[rgba(26,26,26,0.45)]">
                          ...
                          <code className="rounded bg-[#F0E8DA] px-1">
                            name
                          </code>
                          ,{" "}
                          <code className="rounded bg-[#F0E8DA] px-1">
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-[rgba(26,26,26,0.45)]">
                            Auto-fill subject &amp; body from a saved template
                          </p>
                        </div>
                      </div>
                      <ChevronDown
                        className={`h-4 w-4 text-[rgba(26,26,26,0.35)] transition-transform ${showTplPicker ? "rotate-180" : ""}`}
                      />
                    </button>
                    {showTplPicker && (
                      <div className="mt-2 overflow-hidden rounded-xl border border-[#E3D4C2]">
                        {tplLoading ? (
                          ...
                          <p className="px-4 py-4 text-sm text-[rgba(26,26,26,0.45)]">
                              ...
                              >
                                ...
                                <div className="min-w-0">
                                  <div className="flex items-center gap-2">
                                    <p className="truncate text-sm font-medium text-[#1F1C18]">
                                      {t.name}
                                    </p>
                                    {t.isSystemTemplate &&
                                      !t.organizationId && (
                                  ...
                                  {t.subject && (
                                    <p className="mt-0.5 truncate text-[11px] text-[rgba(26,26,26,0.45)]">
                                      {t.subject}
                                    </p>
                                  )}
                                </div>
                      ...
                      selectedMembers.length > 0 && (
                        <div className="flex flex-wrap gap-1 pt-1">
                          {selectedMembers.slice(0, 6).map((m) => (
                            <span
                              key={m.id}
                              className="rounded-full bg-[rgba(200,160,97,0.15)] px-2 py-0.5 text-[10px] text-[#1F1C18]"
                            >
                              {memberLabel(m)}
                            </span>
                          ))}
